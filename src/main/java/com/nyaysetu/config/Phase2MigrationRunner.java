        ...
        try {
            ...
            try {
                ...
                if (e.getMessage().contains("Duplicate") || e.getMessage().contains("already exists")) {
                    log.info("UNIQUE constraint already exists or could not be applied due to existing data (should be cleaned).");
                } else {
                    log.error("Failed to apply UNIQUE constraint: {}", e.getMessage());
                }
            }
